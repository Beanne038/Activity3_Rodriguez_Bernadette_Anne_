...
--Add another column in the item table named item_price.Set the column Price data type to Decimal with 2 decimal points.
ALTER TABLE item ADD COLUMN item_price DECIMAL(20,2);
...
SELECT p.player_name, h.hero_name FROM public.player p
JOIN public.hero h ON p.hero_id = h.hero_id WHERE h.is_active = true;
...
SELECT c.class_name, AVG(p.player_level) AS average_level FROM public.class c
LEFT JOIN public.hero h ON c.class_id = h.class_id
LEFT JOIN public.player p ON h.hero_id = p.hero_id
GROUP BY c.class_name ORDER BY average_level DESC;
